...
class HMM(Generic[TypeT]):
        ...
        with np.errstate(divide='ignore', invalid='raise'):
            ...
            for i in range(max_iter):
                ...
                for j, sequence in enumerate(self.dataset.sequences):
                    ...
                    p_j_at_t, xi = self.forward_backward_expect(encoded_sequence)
    ...
    def expected_state_occupancy_count(
        ...
        """
        p_j_at_t = log_normalize_exp(log_p_forward + log_p_backward, axis=2)
        return p_j_at_t
    ...
    def expected_state_transition_count(
        ...
        xi = log_normalize_exp(log_xi, axis=(1, 2))
        return xi
    ...
    def forward_backward_max(
        ...
        :param encoded_sequence: T x 1 x |V|
        :param p_j_at_t: T x 1 x |POS|
        :param xi:
        :return: log_pi, log_transition, log_emission_T
        """
        T, _, _ = encoded_sequence.shape
        encoded_sequence = np.squeeze(encoded_sequence, axis=1)
        p_j_at_t = np.squeeze(p_j_at_t, axis=1)

        log_pi = np.logaddexp(log_pi, p_j_at_t[0])
        log_transition = np.logaddexp(log_transition, logsumexp(xi, axis=0))

        # V x T @ T x POS = V x POS
        log_emission_T = np.logaddexp(
            log_emission_T,
            log_mat_mul(encoded_sequence.T, np.log(p_j_at_t))
        )
        return log_pi, log_transition, log_emission_T
